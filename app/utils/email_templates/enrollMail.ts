import { Mail } from './email'

const html_template = `
<!DOCTYPE html>
<html lang="en" >
<head>
  <meta charset="UTF-8">
  <title>CodePen - OTP Email Template</title>
  

</head>
<body>
<!-- partial:index.partial.html -->
<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
  <div style="margin:50px auto;width:70%;padding:20px 0">
    <div style="border-bottom:1px solid #eee">
      <a href="" style="font-size:1.4em;color: #00466a;text-decoration:none;font-weight:600">Youmee</a>
    </div>
    <p style="font-size:1.1em">Hi,</p>
    <p>Thank your for enrolling at Youmee! We will review your application and get back to you as soon as possible.</p>
    <h2 style="background: #00466a;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;"></h2>
    <p style="font-size:0.9em;">Regards,<br />Your Youmee team</p>
    <hr style="border:none;border-top:1px solid #eee" />
    <div style="float:right;padding:8px 0;color:#aaa;font-size:0.8em;line-height:1;font-weight:300">
      <p>Youmee Enterprises</p>
      <p>Bangkok, Thailand</p>
    </div>
  </div>
</div>
</body>
</html>
`

export function enrollMail(recipient: string): Mail {
    return {
        to: recipient,
        from: process.env.SENDGRID_SENDER as string,
        subject: 'Thank you for enrolling',
        text: 'test',
        html: html_template,
    }
}
